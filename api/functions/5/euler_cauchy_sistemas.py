...
def euler_cauchy_sistemas(ecuaciones_strs, t0_str, y0_strs, h, n):
    t = evaluar_expresion(t0_str)
    y = [evaluar_expresion(val) for val in y0_strs]
    fs = crear_sistema_funciones(ecuaciones_strs)

    for _ in range(n):
        y_pred = [yi + h * fi(t, y) for yi, fi in zip(y, fs)]
        f_eval_pred = [fi(t + h, y_pred) for fi in fs]
        y = [yi + h / 2 * (fi(t, y) + fpi) for yi, fi, fpi in zip(y, fs, f_eval_pred)]
        t += h
...
def result(ecuaciones_strs, t0, y0_list, h, n):
    try:
        return euler_cauchy_sistemas(ecuaciones_strs, t0, y0_list, h, n)
    except Exception as e:
        print(f"Error: {e}")
        return None
