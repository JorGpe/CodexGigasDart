...
class NewtonEvaluatePolynomialForm extends StatefulWidget {
  const NewtonEvaluatePolynomialForm({super.key});

  @override
  State<NewtonEvaluatePolynomialForm> createState() =>
      _NewtonEvaluatePolynomialFormState();
}

class _NewtonEvaluatePolynomialFormState
    extends State<NewtonEvaluatePolynomialForm> {
  ...
  Future<void> _callNewtonEvaluatePolynomialApi() async {
    setState(() {
      isLoading = true;
      result = [];
      error = '';
    ...
    if (listaX.length < 1) { // Need at least one point for Newton (though practically often 2+)
      setState(() {
        isLoading = false;
    ...
    try {
     ...
     final Uri uri = Uri.parse('$api_ip/interpolacion_newton');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Newton P(x) Evaluation Calculator',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            ElevatedButton(
              onPressed: isLoading ? null : _callNewtonEvaluatePolynomialApi,
              child: isLoading
                  ? const CircularProgressIndicator()
                  : const Text('Evaluate Newton P(x)'),
            ),
            const SizedBox(height: 16.0),
                ...
                children: [
                  if (result[0] != null) // Interpolated value
                    Padding(
                      ...
                      child: Text(
                        "Interpolación P(x): ${(result[0] as num).toStringAsFixed(6)}",
                  ...
                  if (result[1] != null) // Extrapolated value
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4.0),
                      child: Text(
                        "Extrapolación P(x): ${(result[1] as num).toStringAsFixed(6)}",
                        style: const TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold),
                      ),
                    ),
...
Future<Map<String, dynamic>> callNewtonEvaluatePolynomial(
