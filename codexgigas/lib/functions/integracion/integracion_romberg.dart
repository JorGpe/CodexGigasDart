...
class RombergForm extends StatefulWidget {
  const RombergForm({super.key});

  @override
  State<RombergForm> createState() => _RombergFormState();
}

class _RombergFormState extends State<RombergForm> {
  ...
  Future<void> _callRombergApi() async {
    setState(() {
      isLoading = true;
      result = [];
      error = '';
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: isLoading ? null : _callRombergApi,
                child: isLoading
                    ? const CircularProgressIndicator()
                    : const Text('Calcular Integral (Romberg)'),
              ),
              const SizedBox(height: 16.0),
              if (result.isNotEmpty && result[0] != null)
                Column(
                  ...
                  children: [
                    Text(
                      "Valor aproximado de la Integral: ${(result[0])}", // Romberg can be very precise
...
Future<Map<String, dynamic>> callRomberg(
