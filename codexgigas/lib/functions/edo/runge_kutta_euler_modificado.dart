...
class RungeKuttaModificadoForm extends StatefulWidget {
  const RungeKuttaModificadoForm({super.key});

  @override
  State<RungeKuttaModificadoForm> createState() =>
      _RungeKuttaModificadoFormState();
}

class _RungeKuttaModificadoFormState
    extends State<RungeKuttaModificadoForm> {
  ...
  Future<void> _callRungeKuttaModificadoApi() async {
    setState(() {
      isLoading = true;
      result = [];
      error = '';
    ...
    try {
      // IMPORTANT: Adjust if your Flask API URL is different or points to the same logic as Euler-Cauchy
      final Uri uri = Uri.parse('http://127.0.0.1:5000/runge_kutta_modificado');
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: isLoading ? null : _callRungeKuttaModificadoApi,
                child: isLoading
                    ? const CircularProgressIndicator()
                    : const Text('Calcular y(tₙ) con RK (Euler Mod.)'),
              ),
              const SizedBox(height: 16.0),
...
Future<Map<String, dynamic>> callRungeKuttaModificado(
