...
class PredictorCorrectorIterativoForm extends StatefulWidget {
  const PredictorCorrectorIterativoForm({super.key});

  @override
  State<PredictorCorrectorIterativoForm> createState() =>
      _PredictorCorrectorIterativoFormState();
}

class _PredictorCorrectorIterativoFormState
    extends State<PredictorCorrectorIterativoForm> {
  ...
  Future<void> _callPredictorCorrectorIterativoApi() async {
    setState(() {
      isLoading = true;
      result = [];
      error = '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        // Using the title provided, but M=0 might need clarification for exact method
        title: Text(
          'Predictor-Corrector Iterativo (M=0)',
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: isLoading ? null : _callPredictorCorrectorIterativoApi,
                child: isLoading
                    ? const CircularProgressIndicator()
                    : const Text('Calcular y(tₙ) con Predictor-Corrector'),
              ),
              const SizedBox(height: 16.0),
...
Future<Map<String, dynamic>> callPredictorCorrectorIterativoM0(
